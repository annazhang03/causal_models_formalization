           ...
           exists (U: assignments X) (xX: X),
              ...
              /\ (unobs_conditions_on_Z G (g_path X A2 A3 A4 A1 AZ A6 default) U AZ Z
                  ...
                  /\ forall (Ux: assignments X) (LUx: list (assignments X)),
                    ...
                    -> find_value G (g_path X A2 A3 A4 A1 AZ A6 default) u Ux [] = Some x /\ length LUx <= path_length p
...
Proof.
  ...
  { intros U w x' HA1w HU HeqU.
    ...
    destruct HA1w'' as [xA1 HA1w'']. rewrite HA1w''. unfold f_unobs. simpl.
    rewrite <- HxU. apply HeqU. }

  assert (Hw: (forall (w : node) (U: assignments X) (x: X),
                    is_assignment_for U (nodes_in_graph G) = true
                    -> (forall (u4: node), is_assigned A1 u4 = true -> get_assigned_value U u4 = Some x)
  ...
  { (* show that all nodes in A2 evaluate to x (since they are tied to their parents, which becomes a chain that
    ...
    { intros w HA2w. destruct Hexist as [HA2 [HA2' [HA3 [HA3' HA4]]]].
       ...
       assert (Hwmem: In w (u :: l ++ [v])). { apply member_In_equiv. rewrite <- node_in_path_equiv. apply transmitters_in_path with (G := G).
       ...
       apply HA2' in HA2w'. destruct HA2w' as [a [Haxix Haxsub]]. destruct Haxsub as [Haxsub | Haxsub].
       - apply index_exists in Hwmem. destruct Hwmem as [iwp Hiwp].
         ...
         + intros iw w HA2w Hiwp Hiw a Haxix Haxsub.
           ...
           * assert (Ha: is_assigned A1 a = true \/ is_assigned A2 a = true).
       ...
       - apply membership_rev in Hwmem. apply index_exists in Hwmem. destruct Hwmem as [iwp Hiwp].
         clear Hconn. clear HA3'. clear HA2'.
         generalize dependent a. generalize dependent w. generalize dependent iw. induction iwp as [| iwp' IH].
         + intros iw w HA2w Hiwp Hiw a Haxix Haxsub. (* w = v, but [w;a] is sublist, contradicts acyclic *)
  ...
  { destruct Hexist as [HA2 [HA2' [HA2'' [HA3 [HA3' [HA3D HA4]]]]]].
    intros U w Px pax unobsx iw jw xw yw x HU Hu4 HPx Hparx HxU Hx HA3w. unfold S3_nodes_colliders_in_graph in HA3'.
  ...
  { destruct Hexist as [HA2 [HA2' [HA2'' [HA3 [HA3' [HA3D HA4]]]]]].
    ...
    (* either w is in A3, A4, or not in the path *)
    assert (Hw': exists (P: assignments X), find_values G g (find_parents w G) U [] = Some P
                /\ exists (pa: list X), Some pa = get_parent_assignments P (find_parents w G)
                /\ exists (unobs: X), get_assigned_value U w = Some unobs
                /\ find_value G g w U [] = Some (g w (unobs, pa))).
    ...
    - destruct vA3 as [[[iw jw] xw] yw].
      ...
      { apply HA3_equate with (w := w) (Px := Px) (U := U) (x := x). apply HU. apply Hu4. apply HPx. apply Hparx. apply HxU. apply Hx. apply HA3w. }
      rewrite Hvalw.
    ...
    - destruct (get_assigned_value A4 w) as [iw |] eqn:HA4w.
      + assert (HA4w': is_assigned A4 w = true). { apply assigned_is_true. exists iw. apply HA4w. }
        ...
        * (* contradiction, w is not in A3 *) destruct Hcw as [Hcw _]. rewrite HDw in Hcw. inversion Hcw.
        ...
        * destruct Hcw as [pw' [dw' [Hpdw' Hcw]]]. rewrite Hpdw' in HDw. inversion HDw. rewrite H1 in *. rewrite H2 in *. clear H1. clear H2.
          ...
          { apply HA3_equate with (w := cw) (Px := Pcw) (U := U) (x := x). apply HU. apply Hu4. apply HPcw. apply Hparcw. apply HcwU. apply Hcwf. apply HA3cw. }
          rewrite Hfcw.
  ...
  assert (HUA2': forall u4 : node, is_assigned A1 u4 = true -> get_assigned_value U' u4 = Some xX).
  { intros u4 Hu4. rewrite HeqU'. apply node_maps_to_assigned_value.
    assert (HA1w': In u4 (get_sources_in_g_path G (u, v, l))).
    { destruct (member u4 (get_sources_in_g_path G (u, v, l))) as [|] eqn:Hmem.
      - apply member_In_equiv. apply Hmem.
      - apply HA1 in Hmem. rewrite Hu4 in Hmem. discriminate Hmem. }
  ...
  { apply HcondU with (x := xX). apply HUA. apply HUA2'. }

  split. apply Hcond.

  split. intros w Hw'. apply Hw. apply HUA. apply HUA2'. apply Hw'.
  ...
  { destruct (path_into_start (u, v, l) G) as [|] eqn:Hin.
    + (* u <- ...l... <-> v, u is in A2 *)
      ...
      generalize dependent u. generalize dependent A2. generalize dependent A6. induction l as [| h t IH].
      ...
      * intros A6 A2 HA2'' Heqg u Hp Hcyc HuZ Hdesc Hconn HA2 HA2' HA3 HA4 HA1 HeqUx Hin HA2u.
      ...
      + assert (H4: exists (t4: nodes), get_sources_in_g_path G (u, v, l) = u :: t4).
        { unfold get_sources_in_g_path. rewrite Hin. destruct (path_out_of_end (u, v, l) G) as [[|]|] eqn:Hout.
        ...
        apply assigned_is_true. apply assigned_has_value with (L := u :: t4). split. left. reflexivity. apply HA1. }
      ...
      { destruct LUx as [| U1 LUx']. simpl in HeqU. rewrite HeqU. apply HUxA.
        ...
        remember (tl (get_assignment_sequence_from_sources (get_sources_in_g_path G (u, v, l)) U' x)) as Lt.
         ...
         apply list_equiv_assignment_still_assignment with (L := U1 :: LUx') (L' := Lt). apply HeqLUx.
             intros Ut HUt. apply assignment_sequence_U with (S1 := get_sources_in_g_path G (u, v, l)) (U := U') (L := ((hd 0 (get_sources_in_g_path G (u, v, l)),
            x) :: U') :: Lt) (x := x).
             apply HA1bind. apply HUA. right. apply HUt. rewrite HeqU. apply last_mem. }

      assert (HUA1: forall u4 : node, is_assigned A1 u4 = true -> get_assigned_value U u4 = Some x).
      { intros a Ha. rewrite HeqU. apply assignments_seq_nodes_map_to_x with (U := U') (A := get_sources_in_g_path G (u, v, l)).
      ...
      apply HcondU with (x := x). apply HU. apply HUA1.
Qed.
